...
void MUTEX_INIT(MUTEX_T *ref)
{
    *ref = CreateMutex(NULL /*security attr*/, FALSE /*not locked*/, NULL);
    if (!ref)
        FAIL("CreateMutex", GetLastError());
}
void MUTEX_FREE(MUTEX_T *ref)
{
    if (!CloseHandle(*ref))
        FAIL("CloseHandle (mutex)", GetLastError());
    *ref = NULL;
}
...
// nonexistent on Xbox360, simply disable until a better solution is found
    ...
    if (!TerminateThread(*ref, 0))
        FAIL("TerminateThread", GetLastError());
#endif // PLATFORM_XBOX
    *ref = NULL;
}
...
{
    const char *why = (rc == EINVAL) ? "EINVAL" : (rc == EBUSY) ? "EBUSY"
                                              : (rc == EPERM)   ? "EPERM"
                                              : (rc == ENOMEM)  ? "ENOMEM"
                                              : (rc == ESRCH)   ? "ESRCH"
                                              : (rc == ENOTSUP) ? "ENOTSUP"
...
bool_t SIGNAL_WAIT(SIGNAL_T *ref, pthread_mutex_t *mu, time_d abs_secs)
{
    ...
    else
    {
        ...
        if (rc)
        {
            _PT_FAIL(rc, "pthread_cond_timedwait()", __FILE__, __LINE__);
        }
    }
    return TRUE;
}
...
void THREAD_CREATE(THREAD_T *ref, THREAD_RETURN_T (*func)(void *), void *data, int prio /* -3..+3 */)
{
    ...
    //
    {
        ...
        if (rc)
            _PT_FAIL(rc, "pthread_create()", __FILE__, __LINE__ - 1);
    }
#else
        ...
        while (*st_ref < DONE)
        {
            ...
            else
            {
                ...
                if (rc == ETIMEDOUT)
                    break;
                if (rc != 0)
                    _PT_FAIL(rc, "pthread_cond_timedwait", __FILE__, __LINE__ - 2);
            }
        }
    }
