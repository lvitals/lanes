...
lanes.configure = function(settings_)
    ...
    local gen = function(...)
        ...
        for k, v in pairs(opt) do
            ...
            if not validator then
                error(
                (type(k) == "number" and "Unkeyed option: " .. type(v) .. " " .. string_format("%q", tostring(v)) or "Bad '" .. tostring(k) .. "' option"),
                    2)
            else
                opt[k] = validator(v)
            end
        ...
        local priority, globals, package, required, gc_cb = opt.priority, opt.globals, opt.package or package,
            opt.required, opt.gc_cb
        return function(...)
    ...
    lanes.threads = core.threads or
    function() error "lane tracking is not available" end                                 -- core.threads isn't registered if settings.track_lanes is false
