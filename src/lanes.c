...
static THREAD_RETURN_T THREAD_CALLCONV lane_main(void *vs)
{
    ...
    else
    {
        ...
        enum e_status st = (rc == 0) ? DONE : equal_unique_key(L, 1, CANCEL_ERROR) ? CANCELLED
                                                                                   : ERROR_ST;
...
LUAG_FUNC(wakeup_conv)
{
    ...
    year = (int)lua_tointeger(L, -1);
    lua_pop(L, 1);
    lua_getfield(L, 1, "month");
    month = (int)lua_tointeger(L, -1);
    lua_pop(L, 1);
    lua_getfield(L, 1, "day");
    day = (int)lua_tointeger(L, -1);
    lua_pop(L, 1);
    lua_getfield(L, 1, "hour");
    hour = (int)lua_tointeger(L, -1);
    lua_pop(L, 1);
    lua_getfield(L, 1, "min");
    min = (int)lua_tointeger(L, -1);
    lua_pop(L, 1);
...
LUAG_FUNC(configure)
{
    ...
    */
    ...
    lua_pushfstring(
        L, "%d.%d.%d", LANES_VERSION_MAJOR, LANES_VERSION_MINOR, LANES_VERSION_PATCH); // settings M VERSION
